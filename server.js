...
// serve tutto ciò che sta in public/
app.use(cors());
...
io.on('connection', socket => {
  ...
  socket.on('join',({room,nome})=>{
    const st = rooms[room];
    if(!st) { log(`[ERRORE] join stanza inesistente: ${room}`); return; }
    if(st.usedNames.some(n=>n.toLowerCase()===nome.toLowerCase())){
      socket.emit('errore','Nome già in uso!');
...
// invia domanda con indice
function sendNextQuestion(room){
  const st=rooms[room], idx=st.corrente;
...
server.listen(PORT,()=>log(`✅ Server su porta ${PORT}`));
